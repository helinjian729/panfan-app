...
export const getGroups = (params?: { type?: 'created' | 'joined' }) => {
  // 后端返回 { code, message, data: Group[], timestamp }
  return api.get<Group[]>('/groups', { params })
}

// 获取附近可加入的拼饭团
export const getNearbyGroups = () => {
  return api.get<Group[]>('/groups/nearby')
}
...
export const joinGroup = (data: JoinGroupRequest) => {
  return api.post<Group>('/groups/join', data)
}

// 取消拼饭团（仅发起人）
export const cancelGroup = (id: string) => {
  return api.post<void>(`/groups/${id}/cancel`)
}

// 退出拼饭团（成员）
export const leaveGroup = (id: string) => {
  return api.post<void>(`/groups/${id}/leave`)
}

// 获取拼饭团菜品清单
export const getGroupItems = (id: string) => {
  return api.get<GroupItem[]>(`/groups/${id}/items`)
}

// 添加菜品到拼饭团
export const addGroupItem = (id: string, data: { menuItemId: string; quantity: number; note?: string }) => {
  return api.post<GroupItem>(`/groups/${id}/items`, data)
}

// 删除拼饭团菜品
export const deleteGroupItem = (id: string, itemId: string) => {
  return api.delete<void>(`/groups/${id}/items/${itemId}`)
}

// 计算凑单进度
export const calculateGroup = (id: string) => {
  return api.get<GroupCalculate>(`/groups/${id}/calculate`)
}
